...
class Blockchain:
    ...
    def is_consistent(self, current_velocity, consensus_velocity):
        ...
        tolerance = 2  # Define a tolerance level for consistency check
    ...
    def run_simulation(self):
        ...
        for step in range(self.time_steps):
            ...
            if step == 300 and not self.malicious_added:
                self.malicious_vehicles = [
                    DynamicBicycleModel(blockchain,
                        x=60 - 10 * (i + 1),
                        y=10,
                        vx= 8*(i+1),
                        vehicle_id=100 + i
        ...
        self.plot_trajectory(x_history, y_history)
        #self.plot_velocity_and_distance(time_points, v_history, min_distances)
        ...
        for i in range(self.num_followers + 1):
            ...
            # Align v_history[i] with time_points
            v_trimmed = v_history[i][:len(time_points)]
            plt.plot(time_points[:len(v_trimmed)], v_trimmed, label=label, color=colors[i], linewidth=2.5)
