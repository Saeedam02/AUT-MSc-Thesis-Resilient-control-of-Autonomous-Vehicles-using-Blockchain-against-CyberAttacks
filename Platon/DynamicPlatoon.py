...
class LeaderFollowerSimulationWithInnerLoop:
    def __init__(self, num_followers=5, dt=0.01):
        self.dt = dt
        self.time_steps = int(50.0 / dt)  # 50s sim, for example
        ...
        self.k_s   = 0.1   # how strongly we correct spacing error -> v_x_ref
        self.k_v   = 0.1   # optionally might do relative speed
        self.k_ey  = 0.8   # lateral offset -> r_ref
        self.k_epsi= 0.8   # heading error -> r_ref
            ...
            for i in range(self.num_followers + 1):  # This is correct
                if i == 0:  # leader
                    if t < len(x_history[i]):  # Check if t is within bounds
                        plt.plot(x_history[i][t], y_history[i][t], '.r', markersize=10, label='Leader' if t==t_samples[0] else '')
                else:
                    if t < len(x_history[i]):  # Check if t is within bounds
